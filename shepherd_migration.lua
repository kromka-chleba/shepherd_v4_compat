...
local ctx = shepherd_v4_compat.migration_ctx
local mod_name = (ctx and ctx.mod_name) or shepherd_v4_compat.mod_name or "shepherd_v4_compat"
if type(ctx) ~= "table" then
    error("[" .. mod_name .. "] Migration context is not initialized")
end

local core = ctx.core
local ms = ctx.ms
local storage = ctx.storage
local migration_defer_seconds = ctx.migration_defer_seconds
local debug_ctx = ctx.debug_ctx
local sql_map_reader = ctx.sql_map_reader
local process_mapblock = ctx.process_mapblock
local ensure_required_tags_defined = ctx.ensure_required_tags_defined

if not core then
    error("[" .. mod_name .. "] Migration context missing core")
end
if not ms then
    error("[" .. mod_name .. "] Migration context missing mapchunk_shepherd reference")
end
if not storage then
    error("[" .. mod_name .. "] Migration context missing storage")
end
if type(migration_defer_seconds) ~= "number" then
    error("[" .. mod_name .. "] Migration context has invalid migration_defer_seconds")
end
if type(debug_ctx) ~= "table" then
    error("[" .. mod_name .. "] Migration context missing debug_ctx")
end
if type(sql_map_reader) ~= "table" or type(sql_map_reader.iterate_blocks) ~= "function" then
    error("[" .. mod_name .. "] Migration context missing sql_map_reader.iterate_blocks")
end
if type(process_mapblock) ~= "function" then
    error("[" .. mod_name .. "] Migration context missing process_mapblock")
end
if type(ensure_required_tags_defined) ~= "function" then
...
shepherd_migration.request_manual_migration_purge = request_manual_migration_purge
